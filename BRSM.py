...
control_df = None
inv_cov_matrix = None
features = ["Pain_Score_Pre", "Urgency_Score_Pre", "Nocturnal_Frequency_Pre"]
...
def load_data():
    ...
    try:
        ...
        df.rename(columns={
            ...
            "Pain Score (Pre)": "Pain_Score_Pre",
            "Urgency Score (Pre)": "Urgency_Score_Pre",
            "Nocturnal Frequency (Pre)": "Nocturnal_Frequency_Pre",
            "Treated": "Treatment_Status",
            "Pain Score (Post)": "Pain_Score_Post",
            "Urgency Score (Post)": "Urgency_Score_Post",
            "Nocturnal Frequency (Post)": "Nocturnal_Frequency_Post"
        }, inplace=True)

        # Convert Treatment_Status to labels
        df["Treatment_Status"] = df["Treatment_Status"].map({1: "Treated", 0: "Control"})

        # Separate treated and control groups
        treated_df = df[df["Treatment_Status"] == "Treated"].reset_index(drop=True)
        control_df = df[df["Treatment_Status"] == "Control"].reset_index(drop=True)
...
# GUI Setup
root = tk.Tk()
root.title("Balanced Risk Set Matching (BRSM)")
